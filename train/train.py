...
def solve(global_step):
    ...
    # learning reate
    lr = _configure_learning_rate(174, global_step)
...
def restore(sess):
     ...
     if FLAGS.pretrained_model:
        if tf.gfile.IsDirectory(FLAGS.pretrained_model):
		   ...
		   #checkpoint_path = tf.train.latest_checkpoint('./data/pretrained_models/model.ckpt')
        else:
		   #checkpoint_path = tf.train.latest_checkpoint('./data/pretrained_models/model.ckpt')
...
def train():
    ...
    image, ih, iw, gt_boxes, gt_masks, num_instances, img_id = \
        datasets.get_dataset(FLAGS.dataset_name, 
                             FLAGS.dataset_split_name2, 
                             FLAGS.dataset_dir2, 
                             FLAGS.im_batch,
                             is_training=True)

    data_queue = tf.FIFOQueue(capacity=32,
            dtypes=(
...
if __name__ == '__main__':
    print (FLAGS.restore_previous_if_exists)
    train()
